...
$run_id = omni_start_run($argonar_pdo, 'sync-alrisha');
$objs = 0; $links = 0; $err = null;

try {
    // Business(alrisha) root
    $biz_alrisha = omni_upsert_object($argonar_pdo, [
    ...
    foreach ($rows as $r) {
        ...
        $cid = omni_upsert_object($argonar_pdo, [
        ...
        $objs++;
        if (omni_link($argonar_pdo, $cid, $biz_alrisha, 'BELONGS_TO')) $links++;
        ...
        foreach (array_slice($tx_list, 0, 200) as $tx) {
            ...
            $objs++;
            if (omni_link($argonar_pdo, $cid, $tid, 'RECEIVED', ['occurred_at'=>$when])) $links++;
        }
    }
