...
$run_id = omni_start_run($argonar_pdo, 'sync-events');
$objs = 0; $links = 0; $err = null;

try {
    // Only sessions from the last 7 days, to keep cost bounded
    $rows = $argonar_pdo->query(
        "SELECT session_id,
                COALESCE(NULLIF(site,''), 'argonar') AS site,
    ...
    foreach ($rows as $r) {
        ...
        $biz_id  = omni_id_for_ref($argonar_pdo, $biz_ref);
        ...
        $eid = omni_upsert_object($argonar_pdo, [
        ...
        if ($biz_id && omni_link($argonar_pdo, $eid, $biz_id, 'BELONGS_TO', ['occurred_at'=>$r['started_at']])) $links++;

        if (!empty($r['account_id'])) {
            $p_ref = omni_ref('argonar','person',$r['account_id']);
            $p_id  = omni_id_for_ref($argonar_pdo, $p_ref);
            if ($p_id && omni_link($argonar_pdo, $p_id, $eid, 'PARTICIPATED', ['occurred_at'=>$r['started_at']])) {
                $links++;
            }
        }
    }
