 ...
 *
 * Reads: apexcybernet_pdo.accounts
 * Writes: omni_objects (type=Person, business=apexcybernet)
 *         + BELONGS_TO link Person → Business(apexcybernet)
 */

require_once __DIR__ . '/taxonomy.php';
if (!isset($apexcybernet_pdo)) { require_once __DIR__ . '/../../../includes/db.php'; $apexcybernet_pdo = $pdo; }

$run_id = omni_start_run($apexcybernet_pdo, 'sync-accounts');
$objs = 0; $links = 0; $err = null;

try {
    // Ensure Business(apexcybernet) exists first
    $biz_apexcybernet_id = omni_upsert_object($apexcybernet_pdo, [
        'ref'      => 'global:business:apexcybernet',
        'type'     => 'Business',
        'business' => 'apexcybernet',
        'label'    => 'Apex Cybernet',
        'props'    => ['domain' => 'apexcybernet.com', 'kind' => 'tournaments+wallet'],
    ]);
    $objs++;

    // Column discovery (defensive — schema is not in setup.sql)
    $cols = $apexcybernet_pdo->query("SHOW COLUMNS FROM accounts")->fetchAll(PDO::FETCH_COLUMN);
    ...
    $rows = $apexcybernet_pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach ($rows as $r) {
        ...
        $obj_id = omni_upsert_object($apexcybernet_pdo, [
            'ref'          => $ref,
            'type'         => 'Person',
            'business'     => 'apexcybernet',
            'label'        => $label,
        ...
        if (omni_link($apexcybernet_pdo, $obj_id, $biz_apexcybernet_id, 'BELONGS_TO')) $links++;
    }
