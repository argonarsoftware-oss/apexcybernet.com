...
| `argonar-voice.service` | systemd unit — runs the server as `www-data`. |
...
```bash
cd /var/www/argonar.co/voice-signal
sudo ./deploy.sh
```
...
3. Install + enable the `argonar-voice` systemd service
4. Health-check `http://127.0.0.1:3000/rtc/health`

## One-time Apache step

Add this line inside your `argonar.co` HTTPS vhost
(usually `/etc/apache2/sites-available/argonar.co-le-ssl.conf`):

```apache
Include /var/www/argonar.co/voice-signal/apache-voice.conf
```
...
and the file sits inside the Argonar project directory. The file is in
...
```bash
sudo journalctl -u argonar-voice -f
```
...
curl -s https://argonar.co/rtc/health         # via Apache proxy
```
