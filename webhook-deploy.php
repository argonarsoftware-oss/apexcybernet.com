...
// Fix dubious ownership error — set env var to skip ownership check
putenv('GIT_CONFIG_GLOBAL=/dev/null');

exec('git -c safe.directory=' . WEB_ROOT . ' -C ' . WEB_ROOT . ' fetch origin main 2>&1', $output, $returnCode);

if ($returnCode === 0) {
    exec('git -c safe.directory=' . WEB_ROOT . ' -C ' . WEB_ROOT . ' reset --hard origin/main 2>&1', $output, $returnCode);
}
